...
c ..... use blknum to mark an error  (blknum < 0  bad input)
     ...
     &          (timary(ttlind,2).gt.365)) then
          write(*,'(a,i5/a)') ' fatal error: illegal day of year ',
...
c ....... get real number for erosion
     ...
     &        "bad errosion  ",fltary(ttlind, 1),buffr,lch,istat
        ...
        elseif ('FLOD' .eq. cmdary(ttlind)) then
c ....... get integer flag that controls how water is added to the
c ....... water table
          intary(ttlind, 1) = readint(15,buffr,lch,istat)
          if (istat .eq. 0)  write(*,*)
     &        "bad water table flag  ",intary(ttlind, 1),buffr,lch,
     &        istat
          if ((intary(ttlind, 1) .lt. 0) .or.
     &        (intary(ttlind, 1) .gt. 1)) then
            write(*,*) "invalid watertable flag  ", intary(ttlind, 1)
            write(*,*) "setting to 1, water added automatically"
            intary(ttlind, 1) = 1
          endif

        else
c ....... check for valid commands
          if(INDEX('PLTM FRST LAST SENM TFST TLST DRAN ',
...
c ... Check if decsys needs to be updated.
