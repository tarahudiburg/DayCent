...
c ... Compute soil organic matter sums for plotting
      ...
      dfrstc = dleavc + dfbrchc + dlwodc
      frstc = rleavc + frootcj + frootcm + fbrchc + rlwodc + crootc
      fsysc = somtc + woodc + dfrstc + frstc + strucc(SRFC) + 
     &        metabc(SRFC) + som1c(SRFC) + som2c(SRFC)
...
c ... annual plant, CAK - 12/04/2014
     ...
     &            carbostg(FORSYS,UNLABL) + carbostg(FORSYS,LABELD)
      ...
      do 10 iel = 1, nelem
        ...
        dfrste(iel) = dleave(iel) + dfbrche(iel) + dlwode(iel) 
        frste(iel) = rleave(iel) + frootej(iel) + frootem(iel) +
     &               fbrche(iel) + rlwode(iel) + croote(iel)
        fsyse(iel) = somte(iel) + woode(iel) + dfrste(iel) + 
     &               frste(iel) +
...
c ... Added dwd1c2, dwd2c2, and wstduvc2 for dead standing biomass, -mdh 9/20/2018
...
c ... accumulators respectively in simsom. -mdh 7/19/01
     ...
     &         aglivc +  stdedc + bglivcj + bglivcm +
     &         csrsnk(UNLABL) + csrsnk(LABELD) + woodc + 
     &         dfrstc + frstc
...
c ..... being added to the esrsnk(iel) accumulator in simsom. -mdh 7/19/01
     ...
     &                woode(iel) + dfrste(iel) + frste(iel) + 
...
c ... Above and below ground live C/N ratio
      if (aglive(N) .gt. 0.0 .and. bglivej(N) .gt. 0.0 .and.
     &    bglivem(N) .gt. 0.0) then
...
c ..... Delta 13C output
     ...
     &                         metcis(SOIL,UNLABL), dmetc(SOIL))
        dcarbostg(CRPSYS) = del13out(carbostg(CRPSYS,LABELD),
     &                               carbostg(CRPSYS,UNLABL),
     &                               dcarbostg(CRPSYS))
        dcarbostg(FORSYS) = del13out(carbostg(FORSYS,LABELD),
     &                               carbostg(FORSYS,UNLABL),
     &                               dcarbostg(FORSYS))
        dautoresp(CRPSYS) = del13out(arspmth(CRPSYS,LABELD),
     &                               arspmth(CRPSYS,UNLABL),
     &                               dautoresp(CRPSYS))
        dautoresp(FORSYS) = del13out(arspmth(FORSYS,LABELD),
     &                               arspmth(FORSYS,UNLABL),
     &                               dautoresp(FORSYS))
        dhetresp = del13out(respmth(LABELD), respmth(UNLABL), dhetresp)
        dsoilresp = del13out(arspmth(CRPSYS,LABELD) +
     &                       arspmth(FORSYS,LABELD) + respmth(LABELD),
     &                       arspmth(CRPSYS,UNLABL) +
     &                       arspmth(FORSYS,UNLABL) + respmth(UNLABL),
     &                       dsoilresp)
        dbglivc = del13out(bglcisj(LABELD) + bglcism(LABELD),
     &                     bglcisj(UNLABL) + bglcism(UNLABL), dbglivc)
        dbglivcj = del13out(bglcisj(LABELD), bglcisj(UNLABL), dbglivcj)
        dbglivcm = del13out(bglcism(LABELD), bglcism(UNLABL), dbglivcm)
        dfrootc = del13out(frtcisj(LABELD) + frtcism(LABELD),
     &                     frtcisj(UNLABL) + frtcism(UNLABL), dfrootc)
        dfrootcj = del13out(frtcisj(LABELD), frtcisj(UNLABL), dfrootcj)
        dfrootcm = del13out(frtcism(LABELD), frtcism(UNLABL), dfrootcm)
        deloi = del13out(tlittr(SRFC,LABELD), tlittr(SRFC,UNLABL),
     &                   deloi)
        deloe = del13out(tlittr(SOIL,LABELD) + som3ci(LABELD),
     &                   tlittr(SOIL,UNLABL) + som3ci(UNLABL),
     &                   deloe)
      endif
...
c ..... Delta 14C output
     ...
     &                         metcis(SOIL,UNLABL), dmetc(SOIL))
        dcarbostg(CRPSYS) = del14out(carbostg(CRPSYS,LABELD),
     &                               carbostg(CRPSYS,UNLABL),
     &                               dcarbostg(CRPSYS))
        dcarbostg(FORSYS) = del14out(carbostg(FORSYS,LABELD),
     &                               carbostg(FORSYS,UNLABL),
     &                               dcarbostg(FORSYS))
        dautoresp(CRPSYS) = del14out(arspmth(CRPSYS,LABELD),
     &                               arspmth(CRPSYS,UNLABL),
     &                               dautoresp(CRPSYS))
        dautoresp(FORSYS) = del14out(arspmth(FORSYS,LABELD),
     &                               arspmth(FORSYS,UNLABL),
     &                               dautoresp(FORSYS))
        dhetresp = del14out(respmth(LABELD), respmth(UNLABL), dhetresp)
        dsoilresp = del14out(arspmth(CRPSYS,LABELD) +
     &                       arspmth(FORSYS,LABELD) + respmth(LABELD),
     &                       arspmth(CRPSYS,UNLABL) +
     &                       arspmth(FORSYS,UNLABL) + respmth(UNLABL),
     &                       dsoilresp)
        dbglivc = del14out(bglcisj(LABELD) + bglcism(LABELD),
     &                     bglcisj(UNLABL) + bglcism(UNLABL), dbglivc)
        dbglivcj = del14out(bglcisj(LABELD), bglcisj(UNLABL), dbglivcj)
        dbglivcm = del14out(bglcism(LABELD), bglcism(UNLABL), dbglivcm)
        dfrootc = del14out(frtcisj(LABELD) + frtcism(LABELD),
     &                     frtcisj(UNLABL) + frtcism(UNLABL), dfrootc)
        dfrootcj = del14out(frtcisj(LABELD), frtcisj(UNLABL), dfrootcj)
        dfrootcm = del14out(frtcism(LABELD), frtcism(UNLABL), dfrootcm)
        deloi = del14out(tlittr(SRFC,LABELD), tlittr(SRFC,UNLABL),
     &                   deloi)
        deloe = del14out(tlittr(SOIL,LABELD) + som3ci(LABELD),
     &                   tlittr(SOIL,UNLABL) + som3ci(UNLABL),
     &                   deloe)
      endif
...
c ... Argument declarations
      ...
      if (unlabeled .gt. 0.0001 .and. labeled .gt. 0.0001) then
...
c ... Argument declarations
      ...
      if (unlabeled .gt. 0.0001 .and. labeled .gt. 0.0001) then
c        del14out = ((labeled * 1000.0) / (labeled + unlabeled) - 1) *
c     &             1000.0
        del14out = (((labeled / unlabeled) / FRAC_C14) - 1.0) * 1000.0
      else
        del14out = oldval
      endif
