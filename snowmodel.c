...
**    daylength   - length of daylight (hours)
...
**    langleys - converted value for solar radiation
...
float svapor(float);
    ...
    void snowmodel(int jday, float tmax, float tmin, float tavg,
                   ...
                   float petday, float tmelt[2], float *snlq, double srad,
                   float daylength)
    {
 
      float snowtot, add, langleys;
      float c;
/*!!*
...
*!!*/
      ...
      if (*snow > 0.0) {
        ...
        *Esublim = min(*Esublim, petday);
        ...
        /* in proportion. */
        max(*snow -= *Esublim * ((*snow)/snowtot), 0.0);
        max(*snlq -= *Esublim * ((*snlq)/snowtot), 0.0);
...
/*        *Emelt = tmelt[1] * (tmax - tmelt[0]) + b; */
        ...
        /* subroutines in the calculation of snowmelt, cak - 06/18/2009 */
        /* Convert the solar radiation value from W/m^2 to langleys/day */
        langleys =
          (float)(srad * ((daylength * 60.0 * 60.0) / (10000.0 * 4.184)));
        *Emelt = tmelt[1] * (tmax - tmelt[0]) * langleys;
        if (*Emelt < 0) {
          *Emelt = 0.0f;
        }
        if ((*snow - *Emelt) > 0.0) {
