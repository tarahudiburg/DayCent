...
**                   aboveground live carbon in LAI calculation
**
**  INPUTS:
**    aggreenc           - amount of photosynthetic active carbon (gC/m**2)
...
#include "soilwater.h"
    ...
    void initdaily(int month, float biolive, float biodead, float blitter,
                   ...
                   float *totagb, float aggreenc, float *stcrlai,
                   LAYERPAR_SPT layers)
    {
      float pctlive, pctcover, canopyht;
      float convstcr = 3.0f;
/*      float bslimit[MXSWLYR]; */
      int ilyr;
      ...
      *stcrlai = *biomass / CONVLAI;
      /* Use aggreenc, instead of biolive, to get green LAI */
      if (*biomass > 0.0) {
/*        pctlive = biolive / *biomass; */
        pctlive = (aggreenc * 2.5f) / *biomass;
      } else {
        pctlive = 0.0f;
      }
