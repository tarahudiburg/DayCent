...
**    13May2012 (KLK) recoded pptsoil check so wintlit <= pptsoil
**                    will also reset a negative pptsoil to zero
**
**  INPUTS:
                 ...
                 float blitter)
    {
      ...
      if (watrinput > 0.0) { 
        ...
        *wintlit = (0.015f * (watrinput) + 0.0635f) * (float)exp((double)par1);
        ...
        /* recoded the two checks to a single one that endures intercepted water
           does not exceed the source  KLK*/
        if (*wintlit > *pptsoil) {
          *wintlit = *pptsoil;
          *pptsoil  = 0.0f;
        } else {
