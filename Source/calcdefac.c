...
extern void tcalc(float *, float teff[4], float *);

    void calcdefac(int *texture, float *tfunc, float *bgwfunc, float *agdefac,
                   float *bgdefac, float *avgwfps, float *teff, float *rprpet,
                   int *idef, float *ppt, float *snow, float *avgstemp)
    {
      int   ilyr;
      float a, b, c, d;
      float base1, base2;
      float e1, e2;
      float agwfunc, rel_wc[4], avg_rel_wc;
      float krainwfunc;
      ...
      switch (*idef) {
        case 1:
          ...
          if (rel_wc[0] > 1.0) {
            ...
            if (isnan(agwfunc))
            {
              printf("calcdefac: agwfunc is NaN\n");
              printf("calcdefac: rel_wc[0] = %f\n", rel_wc[0]);
              printf("calcdefac: swc[0] = %f\n", (float)layers->swc[0]);
              printf("calcdefac: width[0] = %f\n", layers->width[0]);
              printf("calcdefac: swclimit[0] = %f\n", layers->swclimit[0]);
              printf("calcdefac: fieldc[0] = %f\n", layers->fieldc[0]);
              exit(1);
            }
          }
          ...
          if (avg_rel_wc > 1.0) {
            ...
            *bgwfunc = 1.0f/(1.0f + 30.0f * (float)exp(-9.0 * avg_rel_wc));
            ...
            if (isnan(*bgwfunc))
            {
              printf("calcdefac: bgwfunc is NaN\n");
              exit(1);
            }
          break;
        case 2:
      ...
      *avgstemp = (soil->soiltavg[1] * layers->width[1] + 
      ...
      /* cak - 04/01/04 */
      if (*ppt > 0.5) {
        *agdefac = max(0.000001f, *tfunc * 2.0f);
      } else {
      ...
      *bgdefac = max(0.000001f, *tfunc * *bgwfunc*krainwfunc);
      ...
      if (isnan(*tfunc))
      {
        printf("calcdefac: tfunc is NaN\n");
        exit(1);
      }
      if (isnan(*agdefac))
      {
        printf("calcdefac: agdefac is NaN\n");
        exit(1);
      }
      if (isnan(*bgdefac))
      {
        printf("calcdefac: bgdefac is NaN\n");
        exit(1);
      }
