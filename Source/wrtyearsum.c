...
**             flux, and CH4 oxidation.
**
**  INPUTS:
**    annppt         - Annual precipitation and irrigation (cm)
**    CH4_oxid_year  - Methane oxidation for year (gCH4-C/m^2)
...
#include "soilwater.h"
    ...
    void wrtyearsum(float *time, float *N2O_year, float *NO_year,
                    float *N2_year, float *CH4_oxid_year,
                    double *nit_amt_year, float *annppt)
    {
      ...
      fprintf(files->fp_yearsum, "%8.2f  %12.6f  %12.6f  %12.6f  %12.6f",
              *time, *N2O_year, *NO_year, *N2_year, *CH4_oxid_year);
