...
c ....... Don't allow negative totale to be used in calculations. -mdh 6/22/2017
c         totale = eavail(iel)
          totale = max(0.0, eavail(iel))
...
c ... Don't allow negative totale to be used in calculations. -mdh 6/22/2017
c     totale = eavail(N) + maxNfix
      totale = max(0.0, eavail(N) + maxNfix)
...
cc ... Adjust so that the N/P ratio of the leaves does not exceed the
cc ... observed critical value of 13.5, cak - 04/05/02
c      if (nelem .ge. P) then
c        if (ecfor(LEAF,N)/ecfor(LEAF,P) .gt. maxnp) then
c          ecfor(LEAF,N) = ecfor(LEAF,P) * maxnp
c        endif
c      endif
...
c          call message(' Warning: nutrlm - ')
c          call message('   total N flow exceeds available N')
c          write(*,*) 'eprodl(N)-(eavail(N)+maxNfix) = ',
c     &                eprodl(N) - (eavail(N) + maxNfix)
        endif
...
c ... Don't allow this subroutine to return negative production value. -mdh 6/22/2017
      if (cprodl .le. 0.0) then
c       write(*,*) 'Warning in nutrlm, cprodl =', cprodl
c       write(*,*) 'Resetting cprodl to 0.0'
        cprodl = 0.0
        do 155 iel = 1, nelem
          eprodl(iel) = 0.0
          do 165 ipart = 1, nparts
            eup(ipart,iel) = 0.0
165       continue
155     continue
        plantNfix = 0.0
