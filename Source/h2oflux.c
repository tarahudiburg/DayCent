...
**    drainDay[]     - static array containing the days of the year on which
**                     soil drainage events are scheduled to occur (1-366)
...
**    rainDay        - static array containing the days of the year on which a
...
float hydr_cond(float satcond, float theta, float thetas, float soiltavg,
    ...
    void h2oflux(int jday, int numlyrs, double swc[MXSWLYR],
                 double swcmin[MXSWLYR], float minpot[MXSWLYR],
                 float depth[MXSWLYR], float width[MXSWLYR],
                 ...
                 float ksatdeep)
    {
      ...
      double hpot[MXSWLYR];
      double flux[MXSWLYR];
      double net_flux[MXSWLYR];
      double cond[MXSWLYR];
      double avcond[MXSWLYR];
      double dist[MXSWLYR];
      double theta[MXSWLYR];
      double swcsave[MXSWLYR];
      double amtTranSave[MXSWLYR];
      double iwater;
      double oversat;
      double evapRate;
      double cuminfl;
...
/*      dt_hours = 2; */
      ...
      *soilEvap = 0.0f;
      ...
      /* If desired, delay drainage of each layer to field capacity based */
      /* on the drainlag parameter value.  Drainage occurs only if there is */
      /* no layer (frozen) impeding the flow.  The maximum number of days */
      /* allowed between drainage events is 5. */
