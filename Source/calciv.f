      ...
      if (initre .ne. ' ') then
        do 80 iel = 1, MAXIEL
          if (cerfor(IVAL,FBRCH,iel) .gt. 0.) then
            ...
            char1 = char(ichar(char(iel)) + ichar('0'))
            string = '   cerfor(3,3,' // char1 // ') is <= 0.0'
            call message(string)
          endif
          if (cerfor(IVAL,LWOOD,iel) .gt. 0.) then
            ...
            call message('   There is an error in your tree.100 file.')
            char1 = char(ichar(char(iel)) + ichar('0'))
            string = '   cerfor(3,4,' // char1 // ') is <= 0.0'
            call message(string)
          endif
          if (cerfor(IVAL,CROOT,iel) .gt. 0.) then
            ...
            call message('   There is an error in your tree.100 file.')
            char1 = char(ichar(char(iel)) + ichar('0'))
            string = '   cerfor(3,5,' // char1 // ') is <= 0.0'
            call message(string)
          endif

c ....... Add initialization of standing dead tree pools.
c ....... Use maximum C:E ratio for dead attached leaves,
c ....... dead atttached fine branches, and dead standing  
c ....... large wood. -mdh 9/21/2018.
          if (cerfor(IMAX,LEAF,iel) .gt. 0.) then
            dleave(iel)=dleavc/cerfor(IMAX,LEAF,iel)
c           write(*,*) 'calciv: dleavc =', dleavc
c           write(*,*) 'calciv: dleave(1) =', dleave(1)
c           write(*,*) 'calciv: cerfor(IMAX,LEAF,iel) =', 
c    &                          cerfor(IMAX,LEAF,iel)
          else
            call message('   There is an error in your tree.100 file.')
            char1 = char(ichar(char(iel)) + ichar('0'))
            string = '   cerfor(2,1,' // char1 // ') is <= 0.0'
            call message(string)
          endif
          if (cerfor(IMAX,FBRCH,iel) .gt. 0.) then
            dfbrche(iel)=dfbrchc/cerfor(IMAX,FBRCH,iel)
          else
            call message('   There is an error in your tree.100 file.')
            char1 = char(ichar(char(iel)) + ichar('0'))
            string = '   cerfor(2,3,' // char1 // ') is <= 0.0'
            call message(string)
          endif
          if (cerfor(IMAX,LWOOD,iel) .gt. 0.) then
            dlwode(iel)=dlwodc/cerfor(IMAX,LWOOD,iel)
          else
            call message('   There is an error in your tree.100 file.')
            char1 = char(ichar(char(iel)) + ichar('0'))
            string = '   cerfor(2,4,' // char1 // ') is <= 0.0'
            call message(string)
          endif
80      continue
      endif
