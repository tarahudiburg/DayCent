...
25      continue
        ...
        call ckdata('fertin','feramt',name)
        read(11, *) feramt(P), name 
        call ckdata('fertin','feramt',name)
        read(11, *) feramt(S), name 
        call ckdata('fertin','feramt',name)
        read(11, *) aufert, name 
        call ckdata('fertin','aufert',name)
        read(11, *) ninhib, name 
        call ckdata('fertin','ninhib',name)
        read(11, *) temp, name
        ninhtm = int(temp) 
        call ckdata('fertin','ninhtm',name)
        read(11, *) frac_nh4_fert, name 
        call ckdata('fertin','frac_n',name)
        read(11, *) frac_no3_fert, name 
        call ckdata('fertin','frac_n',name)
        if (feramt(N) .gt. 0.0) then
