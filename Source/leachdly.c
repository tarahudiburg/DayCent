...
#include "swconst.h"
    ...
    void leachdly(float wfluxout[], int numlyrs, double nitrate[],
                  float critflow, float frlechd[], float stream[],
                  float basef, float stormf, double *inorglch)
    {

      int lyr, nxt;
      int iel = 0;  /* nitrogen */
      double amtlea[MXSWLYR];
      double strm_N;
      double base_N;
      double linten;
...
/*        if (wfluxout[lyr] > 1.0E-10) {
          printf("LEACHDLY: wfluxout[%1d] = %12.10f\n",  lyr, wfluxout[lyr]);
        } */
...
/*          printf("critflow = %12.10f\n", critflow);
          printf("linten = %12.10f\n", linten);
          printf("frlechd[N] = %12.10f\n", frlechd[iel]);
          printf("LEACHDLY: nitrate[%1d] = %12.10f\n", lyr, nitrate[lyr]);
          if (amtlea[lyr] > 1.0E-10) {
            printf("LEACHDLY: amtlea[%1d] = %12.10f\n",  lyr, amtlea[lyr]);
          } */
      ...
      stream[iel+1] = stream[iel+1] + (float)(strm_N + base_N);
