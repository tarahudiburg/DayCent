...
**                 flow) (g/m2)
**    is_saturated - flag, 0 = soil is not saturated, 1 = soil is saturated
...
**                           (fraction 0.0-1.0)
**                           (fraction of porespace that is filled with water)
...
#define  M_PI 3.1415926536
    ...
    void denitrify(float *newCO2, double *newNO3, double nitrate[MAXLYR],
                   ...
                   float co2PPM[], double dN2lyr[], double dN2Olyr[],
                   int *jday, int is_saturated)
    {
      int    debug;
      int    ilyr;
      float  a;
...
/*        k[ilyr] = exp(K*layers->dpthmn[ilyr]) -  exp(K*layers->dpthmx[ilyr]); */
      ...
      *newNO3 = 0;
      ...
      for (ilyr=0; ilyr < layers->numlyrs; ilyr++) {
        ...
        /*
          On Wed, 15 Aug 2012 22:52:13 +0000, Keith Paustian wrote:
          I agree - as we talked about the other day, if we simply ‘ratio’
          out the CO2 effect from tillage in the ‘corrected co2’ variable
          in the denitrification subroutine, that is the best solution.
          I don’t think the simulation of decomposition from a tillage
          event per se causes greater N2O from either CO2 effect on O2
          or on electron donors.  We can still get impacts of tillage
          on N2O but those will be from a host of indirect effects.
           Keith
        */
...
/*        wfps_fc[ilyr] = stdfieldc / (1.0f - stdbulkd/(float)PARTDENS); */
        wfps_fc[ilyr] = layers->fieldc[ilyr] /
                        (1.0f - layers->bulkd[ilyr]/(float)PARTDENS);
...
/*        x_inflection = (float)(9.0 - M); */
        ...
        x_inflection *= sitepar->wfpsdnitadj;
...
/*        fDwfps = 0.5 +
        ...
        /* given days of the year, cak - 02/19/04 */
        ...
        /* If the system is flooded use the user specified N2/N2O ratio */
        if (is_saturated == 0) {
          ...
          /* Adjust N2:N2O ratio based on coefficient read from sitepar.in */
          Rn2n2o *= sitepar->n2n2oadj;
          if (Rn2n2o < 0.1) {
            Rn2n2o = 0.1;
          }
          if (debug) {
