...
c ... storage pool
c ... When carbostg(FORSYS,*) goes to zero, set the labeled fraction 
c ... for the new material entering this pool to the labeled fraction
c ... of the current tree, cisoff. -cak 05/08/2014
...
c From Kendrick 8/1/2018
c I have a fix for a bug in the maintenance respiration in treegrow.
c The bug generates a negative C storage, carbostg, when a non-growing tree
c is removed. Negative storage is a fatal error in livrem.
c (this is only the fatal part of a cluster of errors   :-) )

      call lacalc(lai, fbrchc, rlwodc, maxlai, klai)
      rleavc_opt = lai / (2.5 * btolai)

c The error in the maintenance respiration stems from the fact that
c respiration occurs in non-growing trees which removes C from storage but
c C is only added during active growth. A long period of non-growth will
c exhaust the storage pool. The removal can go negative at zero rleavc_opt
c but even if the intercept is zero, round-off error in the allocation of
c small amounts does the rest.
c 
c The fix comes in 2 parts.
c 1. make fmrsplai(2) the minimum value for the storage pool
c impact on respiration. My parameters make that value 0 at 0 leaf
c potential but there is no guarantee in the input of that intercept 
c it is possible to imagine a slope that would give a negative impact 0
c 2. The second fix is to set a maximum respiration of the storage pool
c OLD:          mrspdyflux(FORSYS) = sum(fmrspdyflux)
c NEW:          mrspdyflux(FORSYS) = min(sum(fmrspdyflux), cstor)

C  OLD CODE ==============================================
C
C     if (rleavc_opt > 0.000000001) then
C       if (carbostg(FORSYS,UNLABL)+carbostg(FORSYS,LABELD) .lt.
C    &      fmrsplai(3) * rleavc_opt) then
C         mrspReduce =
C    &      line(carbostg(FORSYS,UNLABL)+carbostg(FORSYS,LABELD),
C    &           fmrsplai(1) * rleavc_opt, fmrsplai(2),
C    &           fmrsplai(3) * rleavc_opt, fmrsplai(4))
C       elseif (carbostg(FORSYS,UNLABL)+carbostg(FORSYS,LABELD) .gt.
C    &          fmrsplai(5) * rleavc_opt) then
C         mrspReduce = fmrsplai(6)
C       else
C         mrspReduce =
C    &      line(carbostg(FORSYS,UNLABL)+carbostg(FORSYS,LABELD),
C    &           fmrsplai(3) * rleavc_opt, fmrsplai(4),
C    &           fmrsplai(5) * rleavc_opt, fmrsplai(6))
C       endif
C     else
C       mrspReduce = 0.0
C     endif
...
c  BEGIN NEW CODE ==============================================
      cstor = carbostg(FORSYS,UNLABL)+carbostg(FORSYS,LABELD) 
      if (rleavc_opt > 0.000000001) then
        if (cstor .le. fmrsplai(1) * rleavc_opt) then    
c ....... Minimum value
          mrspReduce = fmrsplai(2)
        elseif (cstor .ge. fmrsplai(5) * rleavc_opt) then   
c ....... Maximum value
          mrspReduce = fmrsplai(6)
        elseif (cstor .lt. fmrsplai(3) * rleavc_opt) then     
c ....... First segment
...
c ... carbohydrate storage (cstor) -mdh 10/7/2018
      mrspprev = mrspdyflux(FORSYS)
      mrspdyflux(FORSYS) = min(mrspdyflux(FORSYS), cstor)
      if (mrspdyflux(FORSYS) .gt. 0.0) then
        do 56 ipart = 1, FPARTS
          fmrspdyflux(ipart) = fmrspdyflux(ipart) * 
     &      mrspdyflux(FORSYS)/mrspprev
56      continue
      else
        do 57 ipart = 1, FPARTS
          fmrspdyflux(ipart) = 0.0 
57      continue
       endif
c  END NEW CODE ==============================================
